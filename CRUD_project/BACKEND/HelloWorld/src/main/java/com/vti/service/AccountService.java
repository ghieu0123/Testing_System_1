...
public class AccountService implements IAccountService {
	...
	public void updateAccount(UpdatingAccountForm form) {
		...
		Integer beforeDepartmentId = acc.getDepartment().getId();
		Integer afterDepartmentId = form.getDepartmentId();
		
		acc.setDepartment(departmentRepository.getById(afterDepartmentId));
		
		repository.save(acc);
		
		if(beforeDepartmentId != afterDepartmentId) {
			// update totalmember cho befor department	
			Department departmentBefor = departmentRepository.findById(beforeDepartmentId).get();
